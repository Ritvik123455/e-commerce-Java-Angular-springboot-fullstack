...
export class ProductListComponent implements OnInit {
  ...
  handleListProducts(){
    ...
    // if we have a diffeent category id than prev, then set thePageNumber back to 1
    if (this.previousCategoryId != this.currentCategoryId){
      this.thePageNumber = 1;
    }

    this.previousCategoryId =  this.currentCategoryId;
    console.log(`currenctCategoryId=${this.currentCategoryId}, thePageNumber=${this.thePageNumber}`);
